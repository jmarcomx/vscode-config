...
async function handleTestsCollected(testInfo) {
    ...
    const workspaceFolder = (0, common_1.getWorkspaceFolderForUriAndShowInfoIfNotFound)(uri);
...
async function runHandler(shouldDebug, request, token) {
    ...
    while (queue.length > 0 && !token.isCancellationRequested) {
        ...
        let uri = test.uri;
        let workspaceFolder = (0, common_1.getWorkspaceFolderForUriAndShowInfoIfNotFound)(uri);
        if (workspaceFolder) {
